import { Inngest, cron, step } from "inngest";
...
export const inngest = new Inngest({ id: "groceryDelivery-app" });
...
const sendMonthlyOffer = inngest.createFunction(
  ...
  async () => {
    ...
    let sentCount = 0;
    // Send in batches of 10 to avoid overwhelming/overlimiting mail server
            ...
            body: `<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
                ...
                <div style="padding: 28px;">
                    <p style="margin: 0 0 20px; font-size: 15px; color: #374151;">
                        Hi <strong>${
                          u.name
                        }</strong>, check out this month's top picks!
                    </p>
                                                    ...
                                                    <div style="padding: 10px;">
                                                        ...
                                                        <p style="margin: 4px 0 0; font-size: 15px; font-weight: 700; color: #16a34a;">
                                                            ...
                                                            ${
                                                              p.originalPrice >
                                                              p.price
                                                                ? `<span style="font-size: 11px; color: #9ca3af; text-decoration: line-through; margin-left: 4px;">$${p.originalPrice.toFixed(
                                                                    2
                                                                  )}</span>`
                                                                : ""
                                                            }
...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      // Find an active rider who is not currently delivering
      ...
      const busyRiderIds = busyOrders.map((odr) => odr.deliveryPartnerId);
...
export const functions = [checkLowStock, sendMonthlyOffer, autoAssignRider];
